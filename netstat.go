...
var (
	netStatInstance     *NetStat
	netStatInstanceOnce sync.Once
)
...
func obtainNetStat() *NetStat {
	netStatInstanceOnce.Do(func() {
		if nil == netStatInstance {
			netStatInstance = &NetStat{}
		}
	})
	return netStatInstance
}

// Info DiskStats 对象
func (n *NetStat) Info() error {
	return n.readNetStat(gnomon.StringBuild(FileRootPath(), "/net/netstat"))
}

func (n *NetStat) readNetStat(path string) error {
